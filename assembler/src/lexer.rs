        ...
        'lex_loop: while let Some(&character) = input_iter.peek() {
            ...
            match character {
                ...
                'a'..='z' | 'A'..='Z' => {
                    ...
                    // TODO: hashmap might be better for speed(with maybe a custom hash function)
                    let special_identifiers = vec![
                        "add", "sub", "mul", "div", "cmp", "mov", "ldr", "str", "stp", "b", "bl",
                        "cbz", "cbnz", "asr", "x0", "x1", "x2", "x3", "x4", "x5", "x6", "x7", "x8",
                        "x9", "x10", "x11", "x12", "x13", "x14", "x15", "x16", "x17", "x18", "x19",
                        "x20", "x21", "x22", "x23", "x24", "x25", "x26", "x27", "x28", "x29",
                        "x30", "sp", "xzr",
                    ];

                    if special_identifiers.contains(&&*identifier.as_str()) {
                        special_identifiers.iter().for_each(|&inst| {
                            if inst == *identifier {
                                let token = Token {
                                    token_type: match inst {
